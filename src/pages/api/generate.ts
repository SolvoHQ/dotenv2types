...
PROVENANCE HEADER: every output starts with a provenance comment as its first line, so downstream devs opening the file later see where it came from:
- \`schema\` (env.ts) and \`types\` (types-only .ts): first line is \`// Generated by dotenv2types — edit freely\`
- \`example\` (.env.example): first line is \`# Generated by dotenv2types — edit freely\`

OUTPUT 1 — \`schema\` (env.ts):
- Line 1: \`// Generated by dotenv2types — edit freely\`
- Then \`import { z } from "zod";\`
...
- Line 1: \`// Generated by dotenv2types — edit freely\`
- Then a pure \`export type Env = { ... }\` declaration with grouped + commented keys, no Zod, no runtime
...
async function runClaude(apiKey: string, env: string): Promise<Out> {
  ...
  if (
    ...
    throw new Error('Model returned malformed payload — try a smaller input or report at github.com/SolvoHQ/dotenv2types/issues');
  }
  return {
