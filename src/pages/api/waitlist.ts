  ...
  if (apiKey) {
    try {
      const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          to: ['agent+wl-dotenv2types@west0n.top'],
          subject: `dotenv2types waitlist: ${email}`,
    ...
    try {
      const replyText = [
        'Hi,',
        '',
        "Thanks for joining the dotenv2types waitlist — you're in line for v1.2 (secret-detection upgrade + service-grouping UI + .env.example round-trip).",
        '',
        "Quick favor: what's the messiest .env you currently maintain? Just hit reply with a sanitized paste or describe the shape (services / count / pain), and I'll prioritize that pattern when v1.2 ships.",
        '',
        '— dotenv2types',
      ].join('\n');
      const replyRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'dotenv2types@west0n.top',
          to: [email],
          subject: 'Welcome to dotenv2types — quick question',
          text: replyText,
        }),
      });
      if (!replyRes.ok) {
